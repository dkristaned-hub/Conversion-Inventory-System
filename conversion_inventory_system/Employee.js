...
function confirmLogout() {
    ...
    if (!isAdmin && currentUser && currentUser.employeeId) {
        ...
        checkoutBtn.style.display = 'flex'; // Use flex to properly display icon and text
        ...
        document.getElementById('cancelLogoutBtn').onclick = function() {
            ...
            if (checkoutBtn.style.display !== 'none') {
                ...
                logoutBtn.style.display = 'flex'; // Use flex to properly display icon and text
            } else {
                closeModal('logoutModal');
            }
        };
...
function updateAccountsTable() {
    const users = ensureEmployeeIdentifiers(JSON.parse(localStorage.getItem('users')) || []);
    ...
    users.forEach((user, index) => {
        ...
        const employeeIdDisplay = user.employeeId && user.employeeId.trim().length > 0
            ? user.employeeId
            : (user.username || 'N/A');
        ...
        row.appendChild(createCell(employeeIdDisplay));
        row.appendChild(createCell(user.username));
        row.appendChild(createCell(user.fullName));
        row.appendChild(createCell(user.privilege));

        const actionsCell = document.createElement('td');
        actionsCell.className = 'table-cell account-actions-cell';

        const editBtn = document.createElement('button');
        editBtn.className = 'action-button edit';
        editBtn.textContent = 'Edit';
        editBtn.addEventListener('click', () => editAccount(index));
...
function ensureEmployeeIdentifiers(users) {
    ...
    users.forEach(user => {
        ...
        const hasEmployeeId = typeof user.employeeId === 'string' &&
            user.employeeId.trim().length > 0 &&
            user.employeeId.trim().toLowerCase() !== 'n/a';

        const usernameLooksLikeId = typeof user.username === 'string' &&
            /^(STF|ADM)-\d{3,}$/i.test(user.username.trim());
        if (!hasEmployeeId) {
            const privilege = user.privilege === 'Admin' ? 'Admin' : 'Staff';
            const generatedId = usernameLooksLikeId ? user.username.trim().toUpperCase() : generateEmployeeId(privilege);
            user.employeeId = generatedId;
            usersUpdated = true;

            const logEntry = employeeLog.find(entry =>
                entry && entry.name === user.fullName && (!entry.id || entry.id.trim().length === 0 || entry.id === 'N/A')
            );
            if (logEntry) {
                logEntry.id = generatedId;
                logEntry.position = privilege;
                logUpdated = true;
            }
        }
    });
...
function deleteAccount(index) {
    ...
    const passwordInput = document.getElementById('deleteVerifyPassword');
    passwordInput.value = '';
    passwordInput.focus();
    ...
    document.getElementById('confirmDelete').onclick = handleConfirm;
...
function clearAllAccounts() {
  verifyAdminPassword(() => {
    showConfirmationModal(
      'Confirm Clear',
      '⚠️ Are you sure you want to clear ALL accounts? This action cannot be undone!',
      () => {
  ...
  }, 'Verify Admin Password to Clear All Accounts');
}
...
function clearEmployeeLog() {
  verifyAdminPassword(() => {
    showConfirmationModal(
      'Confirm Clear',
      'Are you sure you want to clear the employee log? This action cannot be undone.',
      () => {
        localStorage.removeItem('employeeLog');
        updateEmployeeLogTable();
        updateEmployeeStats();
  ...
  }, 'Verify Admin Password to Clear Employee Log');
}
...
function clearPresentAttendance() {
  verifyAdminPassword(() => {
    showConfirmationModal(
      'Confirm Clear',
      'Are you sure you want to clear the present attendance? This action cannot be undone.',
      () => {
  ...
  }, 'Verify Admin Password to Clear Present Attendance');
}
...
function clearEmployeeShifts() {
  ...
  }, 'Verify Admin Password to Clear Employee Shifts');
}
