...
class Auth {
    ...
    public function login($username, $password) {
        $stmt = $this->conn->prepare("SELECT user_id, username, password, full_name, role, employee_id FROM users WHERE username = ?");
        ...
        if ($result->num_rows == 1) {
            ...
            if (password_verify($password, $user['password'])) {
                ...
                $_SESSION['privilege'] = $user['role'];
                $_SESSION['employee_id'] = $user['employee_id'];

                // Log activity
                $this->logActivity($user['user_id'], $user['username'], 'Login', 'User logged in successfully');
                ...
                if (isset($user['privilege']) && $user['privilege'] === 'Staff') {
                    ...
                    if (!empty($employee_id)) {
                        ...
                        if ($res_check->num_rows == 0) {
                            ...
                            $stmt_att->execute();
                            $this->logActivity($user['user_id'], $user['username'], 'Attendance Check-In', 'Automatic check-in on login');
                        }
                    }
                }
    ...
    private function logActivity($user_id, $username, $activity, $details) {
        ...
        $stmt = $this->conn->prepare("INSERT INTO activity_logs (activity_date, activity_description, conducted_by) VALUES (CURDATE(), ?, ?)");
    ...
    public function register($username, $password, $full_name, $privilege) {
        ...
        // Insert user
        $stmt = $this->conn->prepare("INSERT INTO users (employee_id, username, password, full_name, role) VALUES (?, ?, ?, ?, ?)");
